...
var icu;
try {
    icu = require("node-icu-charset-detector");
} catch (e) {
    icu = null;
}
...
MailParser.prototype._parseDateString = function(value) {
    ...
    if (Object.prototype.toString.call(date) != "[object Date]" || date.toString() == "Invalid Date") {
        try {
            ...
            console.error('Error parsing input %s', value, e);
            return false;
        }
    ...
    if (this._currentNode.content) {
        ...
        if (!this._currentNode.attachment) {
            ...
            if (this._currentNode.meta.contentType == "text/html") {
                this._currentNode.meta.charset = this._detectHTMLCharset(this._currentNode.content, this._currentNode.meta.charset) || this._currentNode.meta.charset || this.options.defaultCharset || "iso-8859-1";
            }
 ...
 *
 * @param {String} HTML html contents
 * @param {String} Charset defined by metaCharset
 * @returns {String} Charset for the HTML
 */
MailParser.prototype._detectHTMLCharset = function(html, metaCharset) {
    var charset, input, meta;

    if (icu) {
        if (!Buffer.isBuffer(html)) {
            input = new Buffer(html, 'binary');
        } else {
            input = html;
        }

        charset = icu.detectCharset(input);
        input = charset.toString().trim().toLowerCase();

        if (charset.confidence > 80 || input === metaCharset) {
            return input;
        }
